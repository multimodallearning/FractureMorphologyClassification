...
from datasets import GrazerFracturesImageDataset_BinaryFractureClassification
from models import ResNetClassifier_BinaryFractureClassification
from utils import MultiLabelMetricFNAdaption

os.environ["CUDA_VISIBLE_DEVICES"]="0"
iou_threshold = 0.5 # minimal threshold to count as fracture
data_folder = "data/preprocessed_images"
label_path = "data/yolo_bbox_labels"
gt_bbox_path = "data/gt_bboxes_label"
save_folder = "data/output"
os.makedirs(save_folder, exist_ok=True)


for confidence_threshold in [0.01]:
    ...
    # Create dataset
    dataset = GrazerFracturesImageDataset_BinaryFractureClassification(data_path=data_folder, label_path=label_path, gt_bbox_path=gt_bbox_path, confidence_threshold=confidence_threshold, iou_threshold=iou_threshold,transform=transform)
    ...
    # save best model
    checkpoint_callback = ModelCheckpoint(monitor="val_f1", mode="max", save_top_k=1, filename="best-f1-{epoch:02d}-{val_f1:.4f}-yolofpfn_conf")

    num_classes = len(set(dataset.labels))
    print(num_classes)

    # Initialize the model
    model = ResNetClassifier_BinaryFractureClassification(num_classes=num_classes, class_weights=class_weights, save_folder=save_folder, pretrained=True)
    ...
    best_model = ResNetClassifier_BinaryFractureClassification.load_from_checkpoint(best_model_path, num_classes=num_classes)
    ...
    fn = torch.bincount(torch.tensor(dataset.fn_labels))
    with open(save_folder+"/fnlabels.txt", "w") as f:
        for i, count in enumerate(fn):
            f.write(f"Class {i}: {count.item()}\n")

    extra_fn = torch.bincount(torch.tensor(dataset.fn_labels))

    metric = MultiLabelMetricFNAdaption(labels, preds, num_classes, labels, extra_fn=extra_fn)

    acc = metric.compute("accuracy")
    f1 = metric.compute("f1")
    precision = metric.compute("precision")
    recall = metric.compute("recall")

    # fracture and no fracture
    morphology_classes = list(range(1))

    lines = []
    lines.append("Metriken pro Klasse:\n")
    for i, class_name in enumerate(morphology_classes):
        lines.append(f"Klasse: {class_name}")
        lines.append(f"  Accuracy : {acc['per_class'][i]:.4f}")
        lines.append(f"  F1 Score : {f1['per_class'][i]:.4f}")
        lines.append(f"  Precision: {precision['per_class'][i]:.4f}")
        lines.append(f"  Recall   : {recall['per_class'][i]:.4f}")
        lines.append("-" * 30)

    lines.append("\nMean over all classes:\n")
    lines.append(f"Accuracy: {acc['macro_avg']:.4f}")
    lines.append(f"F1-Score: {f1['macro_avg']:.4f}")
    lines.append(f"Precision: {precision['macro_avg']:.4f}")
    lines.append(f"Recall: {recall['macro_avg']:.4f}")
