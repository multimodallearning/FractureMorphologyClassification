...
from utils import MORPHOLOGY_AO, MultiLabelMetricFNAdaption
from datasets import GrazerFracturesImageDataset_YOLO
...
for confidence_threshold in [0.01, 0.05, 0.1, 0.5, 0.8, 0.85]:
    ...
    # Create dataset
    dataset = GrazerFracturesImageDataset_YOLO(data_path=data_folder, label_path=label_path, gt_bbox_path=gt_bbox_path, confidence_threshold=confidence_threshold, iou_threshold=iou_threshold, transform=transform)
    ...
    # save best model
    checkpoint_callback = ModelCheckpoint(monitor="val_f1", mode="max", save_top_k=1, filename="best-f1-{epoch:02d}-{val_f1:.4f}-yolofn_conf")
