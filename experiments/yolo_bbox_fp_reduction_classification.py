...
from utils import MORPHOLOGY_AO_FPREDUCTION, MultiLabelMetricFNAdaption
from datasets import GrazerFracturesImageDataset_YOLOFPREDUCTION
...
for confidence_threshold in [0.01, 0.05, 0.1, 0.5, 0.8, 0.85]:

    print("start with " + str(confidence_threshold))
    ...
    # Create dataset
    dataset = GrazerFracturesImageDataset_YOLOFPREDUCTION(data_path=data_folder, label_path=label_path, gt_bbox_path=gt_bbox_path, confidence_threshold=confidence_threshold, iou_threshold=iou_threshold, transform=transform)
    ...
    extra_fn = torch.cat((torch.bincount(torch.tensor(dataset.fn_labels)), torch.tensor([0])))

    metric = MultiLabelMetricFNAdaption(labels, preds, num_classes, labels=labels, extra_fn=extra_fn)
    ...
    morphology_classes = list(MORPHOLOGY_AO_FPREDUCTION.keys())
